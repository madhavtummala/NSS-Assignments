	...
	while True:
		if choice == 1:
			...
			for i in k:
				p=i.rsplit(" ",1)
				if p[0] in subdict:
					subdict[p[0]].append(s[i])
				else:
					subdict[p[0]]=list([s[i]])
			...
			for key,value in ss.items():
				if new_name==key.rsplit(" ",1)[0]:
					count=max(count,key.rsplit(" ",1)[1])
			'''if new_name in ss:
				conn.sendall("This name already exists. Please choose a new name")
				break'''
			new_name=new_name+" "+str(count+1)
			s[num]=new_name
			...
			conn.sendall("Enter the old number")
			old_number=conn.recv(1024)
			with open("database_mob.json","r") as f:
				s=f.read()
			s=json.loads(s)
			if old_number not in s:
				conn.sendall("Couldn't find entry to update")
				break
			old_name=s[old_number]
			with open("database_name.json","r") as f:
				ss=f.read()
			ss=json.loads(ss)
			ss[old_name]=new_number
			with open("database_name.json","w") as f:
