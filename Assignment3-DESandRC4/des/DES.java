...
class DES {
	...
	// S-boxes (i.e. Substitution boxes). Change them to obtain a different encryption altogether
    ...
    static String decryption = "", encryption = "";
	...
	public static void main(String args[]) {
        ...
        int _keyBits[] = new int[64];
          for(int i=0 ; i < 16 ; i++) {
              ...
              for(int j=0 ; j < 4 ; j++) {
                  _keyBits[(4*i)+j] = Integer.parseInt(s.charAt(j) + "");
              }
          }
        int l = input.length();
        ...
        for(int num=0;num<arr.length;num++) {       
          String x=Integer.toHexString(arr[num]);
          //System.out.println(x+","+arr[num]);
          while(x.length()<16)
            x="0"+x;
          input = x;
          ...
          for(int i=0 ; i < 16 ; i++) {
            ...
            int keyBits[]=Arrays.copyOf(_keyBits, _keyBits.length);
            ...
            if(verbose)
              System.out.println("\n+++ ENCRYPTION of block " +(num+1) + " +++");
            int outputBits[] = permute(inputBits, keyBits, false, verbose);
            permute(outputBits, keyBits, true, false);
        }
        System.out.println("\n+++ FINAL ENCRYPTION +++");
	...
	private static int[] permute(int[] inputBits, int[] keyBits, boolean isDecrypt, boolean verbose) {
